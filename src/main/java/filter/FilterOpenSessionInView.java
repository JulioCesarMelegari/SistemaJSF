...
import listener.ContextLoaderListenerAplicacao;
...
public class FilterOpenSessionInView extends DelegatingFilterProxy implements Serializable {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
		...
		// JDBC SPRING
		BasicDataSource springBasicDataSource = (BasicDataSource) ContextLoaderListenerAplicacao
		...
		try {
			request.setCharacterEncoding("UTF-8");// define a condificacao
			...
			sf.getCurrentSession().beginTransaction();// inicia a transacao

			// antes de executar ação (Request)
			super.doFilter(request, response, filterChain); // executa acao no servidor

			// apos a execucao (resposta)
